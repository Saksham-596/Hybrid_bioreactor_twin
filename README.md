...
1. The Physics Engine (scipy.integrate.odeint)Uses a system of ordinary differential equations (ODEs) to calculate the "Theoretical Max" growth curve:$$\mu = \frac{\mu_{max} \cdot S}{K_s + S}$$$$\frac{dX}{dt} = (\mu - D)X$$
2. The ML Reality Engine (XGBoost)A gradient-boosted decision tree pipeline that maps real-time setpoints (Air Head Pressure, Vessel Weight, Oil Flow, etc.) to empirical biomass yield.
3. Feature SelectionInstead of training on all available columns, the model was distilled down to the 8 physical drivers that govern oxygen transfer ($k_L a$) and nutrient metabolism, ensuring the model generalizes to new batches without over-fitting to sensor noise.
